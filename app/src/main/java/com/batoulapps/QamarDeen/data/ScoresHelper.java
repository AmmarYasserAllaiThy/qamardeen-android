...
public class ScoresHelper {
    ...
    private static Map<Long, Integer> initializeEntries(Cursor cursor, long maxDate, long minDate) {
        ...
        while (localMaxCal.after(localMinCal) || localMaxCal.equals(localMinCal)) {
    ...
    public static ScoreResult getPrayerScores(QamarDbAdapter dbAdapter, long maxDate, long minDate) {
        ...
        SparseArray<Integer> stats = new SparseArray<>();
        if (cursor != null) {
            ...
            if (cursor.moveToFirst())
                do {
                    ...
                    if (status != QamarConstants.PrayerType.NOT_SET) {
                        int typeCount = stats.get(status, 0) + 1;
                    ...
                    if (status != QamarConstants.PrayerType.NOT_SET) {
                        if (prayer != QamarConstants.Prayers.DUHA && prayer != QamarConstants.Prayers.QIYYAM)
                            totalFara2id++;
                        totalPrayers++;
                    }
    ...
    public static ScoreResult getSadaqahScores(QamarDbAdapter dbAdapter, long maxDate, long minDate) {
        Map<Long, Integer> scores = null;
        ...
        if (cursor != null) {
            ...
            if (cursor.moveToFirst())
                do {
                    ...
                    int typeCount = stats.get(sadaqahType, 0) + 1;
    ...
    public static ScoreResult getFastingScores(QamarDbAdapter dbAdapter, long maxDate, long minDate) {
        Map<Long, Integer> scores = null;
        ...
        if (cursor != null) {
            ...
            if (cursor.moveToFirst())
                do {
                    ...
                    int typeCount = stats.get(fastingType, 0) + 1;
                    ...
                    Integer score = scores.get(localTimestamp);
                    if (score == null) score = 0;
                    if (score == 0 && fastingType != QamarConstants.FastingTypes.NONE) uniqueDays++;
    ...
    public static ScoreResult getQuranScores(QamarDbAdapter dbAdapter, long maxDate, long minDate) {
        Map<Long, Integer> scores = null;
    ...
    public static ScoreResult getOverallScores(QamarDbAdapter dbAdapter, long minDate, long maxDate) {
        ...
        int totalPrayers = prayerResults.statistics.get(QamarConstants.TOTAL_ACTIONS_DONE, 0);
        stats.put(1, totalPrayers);

        ScoreResult quranResults = getQuranScores(dbAdapter, minDate, maxDate);
        scores = updateScoreTotals(scores, quranResults.scores);
        int ayatRead = quranResults.statistics.get(QamarConstants.TOTAL_ACTIONS_DONE, 0);
        stats.put(2, ayatRead);

        ScoreResult sadaqaResults = getSadaqahScores(dbAdapter, minDate, maxDate);
        scores = updateScoreTotals(scores, sadaqaResults.scores);
        int totalSadaqaDays = sadaqaResults.statistics.get(QamarConstants.TOTAL_ACTIVE_DAYS, 0);
        stats.put(3, totalSadaqaDays);

        ScoreResult fastingResults = getFastingScores(dbAdapter, minDate, maxDate);
        scores = updateScoreTotals(scores, fastingResults.scores);
        int totalFastingDays = fastingResults.statistics.get(QamarConstants.TOTAL_ACTIVE_DAYS, 0);
    ...
    private static Map<Long, Integer> updateScoreTotals(Map<Long, Integer> overall,
                                                        Map<Long, Integer> additional) {
        Map<Long, Integer> current;
        Map<Long, Integer> toAdd;
    ...
    private static int getPrayerScore(int prayer, int status) {
        ...
        switch (status) {
            case QamarConstants.PrayerType.ALONE:
                if (prayer == QamarConstants.Prayers.DUHA) score = 200;
                else if (prayer == QamarConstants.Prayers.QIYYAM) score = 300;
                else score = 100;
                break;
            ...
            case QamarConstants.PrayerType.GROUP:
                if (prayer == QamarConstants.Prayers.QIYYAM) score = 300;
                else score = 400;
                break;
    ...
    private static int getSadaqahScore(int sadaqahType) {
        return sadaqahType == QamarConstants.CharityTypes.SMILE ? 25 : 100;
    }
